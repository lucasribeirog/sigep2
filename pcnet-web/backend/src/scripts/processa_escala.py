...
def extrair_medidas(caminho_imagem):
    try:
        ...
        # Sem calibração confiável, não é metodologicamente seguro inventar uma escala.
        if not pixels_per_mm or pixels_per_mm < 1:
            return {"erro": "Calibração automática inconclusiva: blocos métricos da régua não foram detectados com confiança."}
        ...
        if comprimento_total_mm is None:
            return {"erro": "Contorno do objeto inconclusivo para medição automática."}

        return {
            "comprimento_total": str(comprimento_total_mm),
            # O comprimento do cano não deve ser inferido por proporção geométrica genérica.
            "comprimento_cano": ""
        }
