...
export default function FormPatrimonio({ form, onChange, unidades = [], unidadePadrao = '' }) {
    ...
    useEffect(() => {
        if (!form.unidade_custodia && unidadePadrao) setFieldValue('unidade_custodia', unidadePadrao);
    }, [unidadePadrao]);
    ...
    return (
        <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 space-y-6">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="tipo_objeto" value={form.tipo_objeto} onChange={onChange} required className="w-full p-2.5 border border-gray-300 rounded-lg bg-white text-sm">
                    ...
                    <div>
                        ...
                        <select name="resultado_eficiencia" value={form.resultado_eficiencia} onChange={onChange} required className="w-full p-2.5 border border-gray-300 rounded-lg bg-white text-sm">
                ...
                <div className="bg-amber-50/60 p-4 rounded-xl border border-amber-200 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                        <div>
                            ...
                            <input required type="text" name="secao_madeira" value={form.secao_madeira} onChange={onChange} placeholder="Ex: retangular" className="w-full p-2 border border-amber-200 rounded-md bg-white text-sm" />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-amber-900 mb-1">1ª dimensão da seção (mm) *</label>
                            <input required type="number" name="comp_madeira" value={form.comp_madeira} onChange={onChange} placeholder="Ex: 40" className="w-full p-2 border border-amber-200 rounded-md bg-white text-sm" />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-amber-900 mb-1">2ª dimensão da seção (mm) *</label>
                            <input required type="number" name="larg_madeira" value={form.larg_madeira} onChange={onChange} placeholder="Ex: 30" className="w-full p-2 border border-amber-200 rounded-md bg-white text-sm" />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-amber-900 mb-1">Comprimento total (mm) *</label>
                            <input required type="number" name="comp_total" value={form.comp_total} onChange={onChange} placeholder="Ex: 500" className="w-full p-2 border border-amber-200 rounded-md bg-white text-sm" />
                        </div>
                        <div>
                            <label className="block text-xs font-semibold text-amber-900 mb-1">Massa (gramas) *</label>
                            <input required type="number" name="massa" value={form.massa} onChange={onChange} placeholder="Ex: 350" className="w-full p-2 border border-amber-200 rounded-md bg-white text-sm" />
                        </div>
                    </div>
                </div>
            )}
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label className="block text-xs font-semibold text-gray-600 mb-1">Nº do Lacre <span className="font-normal text-gray-400">(opcional)</span></label>
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-gray-600 mb-1">Nº do FAV <span className="font-normal text-gray-400">(opcional)</span></label>
                        <input type="text" name="n_fav" value={form.n_fav} onChange={onChange} placeholder="Detectado do DOCX-base quando possível" className="w-full p-2 border border-gray-300 rounded-lg text-sm bg-white" />
                    </div>
                    <div>
                        <label className="block text-xs font-semibold text-gray-600 mb-1">Unidade de Custódia *</label>
                        <select required name="unidade_custodia" value={form.unidade_custodia} onChange={onChange} className="w-full p-2 border border-gray-300 rounded-lg text-sm bg-white">
                            <option value="">-- Selecione --</option>
                            {unidades.map(u => <option key={u.id || u.nome} value={u.nome}>{u.codigo_externo ? `${u.codigo_externo} — ` : ''}{u.nome}</option>)}
                            {form.unidade_custodia && !unidades.some(u => u.nome === form.unidade_custodia) && <option value={form.unidade_custodia}>{form.unidade_custodia}</option>}
                        </select>
                    </div>
                </div>
            </div>
