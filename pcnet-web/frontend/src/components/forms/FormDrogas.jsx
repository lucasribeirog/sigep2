...
export default function FormDrogas({ dados, onChange }) {
    ...
    const handleDrogaChange = (e) => {
        const valor = e.target.value;
        onChange(e);
        if (valor === 'maconha') onChange({ target: { name: 'cor_material', value: '' } });
        else if (!cor_material) onChange({ target: { name: 'cor_material', value: 'branca' } });
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">
                        Cor / aspecto do material {isCocaina ? '*' : <span className="font-normal text-gray-400">(opcional)</span>}
                    </label>
                    <input
                        type="text"
                        name="cor_material"
                        value={cor_material}
                        onChange={onChange}
                        list="cores-material-droga"
                        placeholder={isCocaina ? 'Ex.: branca' : 'Ex.: esverdeada; deixe vazio se não desejar descrever'}
                        className="w-full border border-gray-300 rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none bg-white"
                        required={isCocaina}
                    />
                    <datalist id="cores-material-droga">
                        {['branca','esbranquiçada','bege','amarronzada','acinzentada','amarelada','esverdeada','verde-amarronzada'].map(c => <option key={c} value={c} />)}
                    </datalist>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">FAV principal <span className="font-normal text-gray-400">(opcional)</span></label>
                    <input
                        type="text"
                        name="numero_fav"
                        value={numero_fav}
                        onChange={onChange}
                        placeholder="Preenchida automaticamente a partir do DOCX-base quando possível"
                        className="w-full border border-gray-300 rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none bg-white"
                    />
                </div>
                <div>
            ...
            {!isFragmentado && (
                <div className="bg-blue-50 p-4 rounded-md border border-blue-100">
                    <label className="block text-sm font-semibold text-blue-900 mb-1">Nº do Lacre de Encaminhamento/Guarda *</label>
                    <input
                        type="text"
            ...
            {isFragmentado && (
                <div className="bg-orange-50 p-4 rounded-md border border-orange-200 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div>
                            <label className="block text-xs font-semibold text-orange-800 mb-1">Nº do Lacre da Amostra *</label>
                            <input
                                type="text"
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-orange-900 mb-1">Nº do Lacre do Restante (Guarda) *</label>
                        <input
                            type="text"
                            name="envelope_encaminhamento"
                            value={envelope_encaminhamento}
